...
resource "oci_core_subnet" "prow_worker_cluster" {
  count = length(data.oci_identity_availability_domains.availability_domains)

  availability_domain = data.oci_identity_availability_domains.availability_domains[count.index].name
